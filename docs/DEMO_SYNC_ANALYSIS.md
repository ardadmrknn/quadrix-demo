...
> **Güncelleme (2026-05-18):** Drift sadece commit sayısıyla değil **architecture seviyesinde** de mevcut. Main, demo fork'undan sonra MysteryCardManager'da büyük bir refactor yaptı: `card_xp`/`card_level`/`compute_xp_award`/`_compute_xp_to_next` ve `RARITY_WEIGHT_ANCHORS`/`_rarity_weights_for_level` API'leri eklendi; `get_mystery_level_fall_speed_ms` modül-level fonksiyonu çıkarıldı. Demo hala eski `progress/threshold` modelinde. Aynı şekilde main'de `focus_manager.py` (gamepad full-support Faz 2), `line_clear_feedback.py` (5 game class arasında ortak helper) ve `settings_screen_tabbed.py`'de keybind modernization (hold-to-clear, conflict detection, slot reset) eklendi. Demo'daki `game._start_block_fall_animation` algoritması da main'den farklı kaldı (`current_offset`/`sweep_trigger` farklı). Bu architectural drift olmayan additive transfer'ler dışında otomatik backport mümkün değil.
...
| `tests/test_mystery_card_quality_gaps.py` | **Deferred — architecture migration gerekli (kod doğrulamalı, 2026-05-18)** — Test `card_xp/card_level/compute_xp_award/_compute_xp_to_next` API'sini ve `pending_level_ups` queue overflow davranışını sınar. Demo `MysteryCardManager` (game_modes_extra.py satır 436-440 init, satır 481 `notify_lines_cleared(cleared)` 1-arg signature) hala eski `progress/threshold` modelinde. Main'de `notify_lines_cleared(cleared, *, source='player', combo=0, back_to_back=False, perfect_clear=False)` keyword-only signature. Kelimesi kelimesine: signature break, source threading her call site'a yayılmalı, `card_xp`/`card_level`/`card_xp_to_next` capture/restore listelerine eklenmeli (AGENTS.md §7 invariant 3). Tam port AGENTS.md §7 invariant 1 (board.level ≠ card.level), 2 (external clear → no XP), 3 (restore queue korunumu), 4 (tek reward → tek prepare_selection) hepsini yeniden doğrulamayı gerektirir. **Bilinçli test taşımama gerekçesi**: bounded refactor değil, full architectural migration. Demo gating (`DEMO_CARD_SELECTION_LEVEL_INTERVAL`, `_demo_score_cap_value`) eski modele bağlı; partial port demo davranışını bozar. |
| `tests/test_mystery_card_rarity_curve.py` | **Deferred — architecture migration gerekli (kod doğrulamalı, 2026-05-18)** — `RARITY_WEIGHT_ANCHORS` ve `_rarity_weights_for_level` classmethod'unu sınar. Demo `MysteryCardManager` bu sabitleri/method'ları içermiyor (kart ağırlıkları farklı algoritmayla seçiliyor). Bu testin geçmesi yukarıdaki tam mystery rework'ün parçasıdır. **Bilinçli test taşımama gerekçesi**: aynı architectural migration zincirinin bağımlısı. |
| `tests/test_mystery_card_xp_progression.py` | **Deferred — architecture migration gerekli (kod doğrulamalı, 2026-05-18)** — `MysteryCardManager.compute_xp_award`, anti-farm (`source='card'/'ability'/...` no XP), pending_level_ups overflow ve time capsule capture/restore'da `card_xp/card_level/card_xp_to_next` korunumunu sınar. Demo'da bu API'lerin hiçbiri yok ve mevcut capture/restore yalnız `progress/threshold` taşıyor (game_modes_extra.py satır 9258-9261). **Bilinçli test taşımama gerekçesi**: aynı architectural migration zincirinin bağımlısı; capture listesi delik bırakırsa AGENTS.md §7 invariant 3 ihlal edilir. |
| `tests/test_mystery_speed_curve.py` | **Deferred — architecture migration gerekli (kod doğrulamalı, 2026-05-18)** — Modül-level `get_mystery_level_fall_speed_ms(level)` ve `MYSTERY_LEVEL_SPEED_ANCHORS` sabitini sınar. Demo'da bu fonksiyon yok; demo `MysteryMode._get_mystery_base_fall_speed` aynı amacı farklı (anchor tablosu yok) implement ediyor. Tam port speed curve sabitlerini ve interpolation'ı eklemek + `get_initial_speed`/`get_current_speed` çağrılarını yeniden noktalamak demek. Mystery rework ile birlikte yapılmalı. **Bilinçli test taşımama gerekçesi**: aynı architectural migration zincirinin bağımlısı. |
