...
DEFAULT_UI_SCALE_PRESET = 'normal'
VALID_UI_SCALE_PRESETS = {'compact', 'normal', 'large', 'huge', 'massive', 'double'}
...
class SettingsManager:
    ...
    def __init__(self, filename='settings.json'):
        ...
        self.default_settings = {
            ...
            'fps_limit': 0,
            # Varsayılan UI ölçeği.
            'ui_scale_preset': DEFAULT_UI_SCALE_PRESET,
            'show_ghost': True,
            'bg_transparency': 0.7,
    ...
    def _load_single_file_settings(self):
        auto_lang = self._get_steam_or_system_language()
        if os.path.exists(self.filename):
            try:
                with open(self.filename, 'r', encoding='utf-8') as f:
                    loaded = json.load(f)
                    if 'language' not in loaded:
                        loaded['language'] = auto_lang
    ...
    def _load_split_settings(self):
        ...
        auto_lang = self._get_steam_or_system_language()
        first_time = not os.path.exists(self.cloud_filename) or 'language' not in (cloud_payload if isinstance(cloud_payload, dict) else {})
        ...
        defaults_copy = copy.deepcopy(self.default_settings)
        defaults_copy['language'] = auto_lang
    ...
    def _get_steam_or_system_language(self) -> str:
        """Steam dilini veya sistem dilini algılar, desteklenmiyorsa 'en' döner."""
        _VALID_LANGUAGES = {'tr', 'en', 'de', 'fr', 'es', 'it', 'pt', 'ru', 'ja', 'zh', 'ko'}

        # 1. Steam Aktif ise Steam Dilini Kullan
        try:
            import steam_integration
            if steam_integration.is_available():
                steam_lang = steam_integration.get_current_game_language_code()
                if steam_lang in _VALID_LANGUAGES:
                    if constants.DEBUG_MODE:
                        print(f"[SettingsManager] Steam dili algilandi: {steam_lang}")
                    return steam_lang
                else:
                    # Steam aktif ama dil desteklenmiyorsa veya None ise sistem diline geçmeden doğrudan 'en'
                    if constants.DEBUG_MODE:
                        print(f"[SettingsManager] Steam dili desteklenmiyor veya alinamadi, 'en' fallback yapiliyor")
                    return 'en'
        except Exception as e:
            if constants.DEBUG_MODE:
                print(f"[SettingsManager] Steam dili alinamadi: {e}")

        # 2. Fallback: Sistem Dili (Yalnızca Steam aktif değilse)
        try:
            import locale
            sys_lang = None
            try:
                # locale.getlocale() can fail on certain platforms or configurations
            ...
            if not sys_lang:
                try:
                    # Fallback for older versions or misconfigured locales
                    sys_lang, _ = locale.getdefaultlocale()
                except AttributeError:
                    pass

            if sys_lang:
                sys_lang = sys_lang.split('_')[0].lower()
                if sys_lang in _VALID_LANGUAGES:
                    if constants.DEBUG_MODE:
                        print(f"[SettingsManager] Sistem dili algilandi: {sys_lang}")
                    return sys_lang
        except Exception as e:
            if constants.DEBUG_MODE:
                print(f"[SettingsManager] Sistem dili alinamadi: {e}")
    ...
    def _sync_ui_scale_preset(self):
        ...
        preset = self._normalize_ui_scale_preset_value(
            self.settings.get('ui_scale_preset', self.default_settings.get('ui_scale_preset', DEFAULT_UI_SCALE_PRESET))
        )
