...
def _setup_dll_functions(dll: ctypes.CDLL) -> None:
    ...
    # ISteamApps_GetCurrentGameLanguage — Steam'de oyun için seçili dili al
    try:
        dll.SteamAPI_ISteamApps_GetCurrentGameLanguage.restype = ctypes.c_char_p
        dll.SteamAPI_ISteamApps_GetCurrentGameLanguage.argtypes = [ctypes.c_void_p]
    except AttributeError:
        pass
...
def get_current_game_language() -> str | None:
    """Steam'de oyun için seçili dili döndürür (örneğin 'english', 'turkish', 'japanese')."""
    if not is_available() or not _isteam_apps or not _dll:
        return None
    try:
        raw = _dll.SteamAPI_ISteamApps_GetCurrentGameLanguage(_isteam_apps)
        if raw:
...
def get_current_game_language_code() -> str | None:
    """Steam oyun dilini 2 harfli kod olarak döndürür ('tr', 'en', vb.)."""
    lang = get_current_game_language()
    if not lang:
        return None
