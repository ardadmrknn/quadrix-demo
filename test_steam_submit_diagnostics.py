...
class TestSubmitScoreDiagnostics(unittest.TestCase):
    ...
    def test_no_license_message_logged(self):
        """Lisanssız hesapta (is_app_owned=False) uygun uyarı mesajı loglanmalı."""
        ...
        license_warn = any('lisansi yok' in msg for msg in logged)
        self.assertTrue(license_warn, f"Lisans uyarı mesajı bulunamadı. Loglar: {logged}")

    def test_fallback_disabled_message_when_sdk_fails_but_might_be_owned(self):
        """Fallback kapalı + is_app_owned=None ise 'fallback kapali' mesajı loglanmalı."""
        m = self._import_module()
        ...
        fallback_warn = any('fallback kapali' in msg or 'STEAM_PARTNER_WRITE_FALLBACK' in msg for msg in logged)
        self.assertTrue(fallback_warn, f"Fallback kapalı mesajı bulunamadı. Loglar: {logged}")
