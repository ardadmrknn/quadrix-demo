...
Tarih: 2026-03-09
Durum: Aktif plan, Faz 8 helper/tutorial migration'i, main popup bridge zinciri ve ilk mod-bazli panel/overlay hardening dalgalari ortak scaling'e tasindi; capraz regresyonlar gecti ve kalan is diger mod layout sabitleri ile olasi tutorial callout polish'inde toplandi
...
Guncel durum (2026-04-05 / Faz 8 kapanis):
...
- `src/main.py` icindeki `_fullscreen_popup_scale()` compatibility wrapper'i ortak `get_scale(...)` yardimcisina baglandi; mode intro, zen start ve tutorial prompt popup zincirinde backdrop yakalama/recover senkronu aktif surface boyutuna hizalandi.
...
- `src/game.py` icinde `update_fonts()`, `get_cell_size()`, `get_board_offset()`, line-clear particle merkezleri, combo/milestone header konumlari, achievement kutusu ile ambient/confetti/firework varsayilan yayilimlari aktif canvas boyutuna hizalandi.
...
- `src/game_modes_extra.py` icinde Mystery board geometri helper'lari, active-cards/perk panel fallback'leri, Sniper dim overlay + talimat paneli, kart atolyesi / parca secim popup'lari ve kart/nova/XP efekt partikulleri aktif canvas ve mevcut board helper zincirine hizalandi.
- `src/game_modes.py` icinde Zen mode satir temizleme ve auto-clear partikulleri stale pencere merkezi yerine aktif canvas merkezinden turetilir hale getirildi.
- `src/tutorial.py` icindeki tutorial başarı kutlama partikulleri de aktif canvas merkezine tasinarak tutorial runtime tarafindaki son stale-center yolu kapatildi.
- Regression kilitleri olarak `tests/test_phase8_overlay_ui_scaling.py`, `tests/test_phase8_tutorial_ui_scaling.py` ve `tests/test_phase8_main_popup_ui_scaling.py` genisletildi; bagimsiz audit sonrasi `src/game_modes_extra.py` icindeki minimum `800x600` Mystery workshop popup tasmasi aktif yukseklik butcesiyle kapatildi, main popup loop'lari, display-surface refresh helper'i ve tutorial canli draw yuzeyleri smoke/regression testleriyle kapsandi, Faz 6-8 birlesik UI scaling paketi `55 passed`, Faz 3-8 + ESC/pause capraz paketi `87 passed`, tam pytest paketi ise `563 passed, 6 skipped` ile gecti.
...
| Faz 8 - Campaign / gameplay overlay ailesi | Tamamlandi   | `src/game.py` temel `_ui_scale()` helper'i aktif canvas tabanli ortak `get_scale(...)` matematiğine tasindi; audit ve kapanis turlarinda pause/HUD/game-over draw anchor'lari, geometri helper'lari, combo/milestone/achievement konumlari ve ambient/confetti/firework varsayilanlari aktif canvas'a hizalandi. `src/game_modes.py` Sprint/Ultra sayaç helper'lari ortak scale zincirine baglandi; `HardcoreMode._draw_right_hud_panel(...)` override'i ile Zen mode satir temizleme/auto-clear particle merkezleri aktif canvas boyutuna tasindi. `src/game_modes_advanced.py` Survival panel helper'i ortak scale zincirine baglandi; Survival panel rect'i aktif canvas icinde clamp'lendi, antivirus flash ve victory/game-over partikulleri aktif canvas merkezine hizalandi, Daily fog overlay'i de aktif surface boyutuna cekildi. `src/game_modes_extra.py` Mystery overlay helper'i ortak scale zincirine baglandi; canli `MysteryMode` font/overlay yolu sabit `1366x768` baseline + aktif canvas ile sertlestirildi, sonraki dalgalarda board geometri helper'lari, active-cards/perk panel fallback'leri, Sniper dim overlay + instruction paneli, kart atolyesi / parca secim popup'lari ve kart/nova/XP efekt partikulleri aktif canvas + mevcut board helper akisina alindi; bagimsiz audit revizyonunda minimum `800x600` workshop popup tasmasi aktif yukseklik butcesiyle kapatildi. `src/tutorial.py` icindeki tutorial overlay/hub/lesson-result/kart-secim yuzeyleri ortak modal scale wrapper'ina alindi; resize yolu olusturulan surface boyutunu geri okuyacak sekilde senkronlandi ve tutorial başarı kutlama partikulleri aktif canvas merkezine tasindi. `src/main.py` popup bridge zincirinde `_fullscreen_popup_scale()` ortak helper'a baglandi ve popup backdrop'u recover/resize sonrasinda aktif surface ile senkronlanir hale getirildi; son audit turunda main popup loop smoke testleri ve tutorial canli draw regression'lari eklendi. Faz 6-8 birlesik UI scaling paketi `55 passed`, Faz 3-8 + ESC/pause capraz paketi `87 passed`, tam pytest paketi `563 passed, 6 skipped`. |
...
- Faz 8 bagimsiz audit revizyonu tamamlanarak kapanmistir; helper dalgasi, tutorial modal/layout dalgasi, `src/main.py` popup bridge zinciri ve gameplay overlay/panel/popup/particle hardening dalgalari ortak scaling + aktif surface senkronuna tasinmistir.
...
- Ortak helper artik settings_screen_tabbed, campaign modal ailesi, campaign HUD ailesi, gameplay overlay helper katmani, tutorial modal/layout ailesi ve `src/main.py` popup bridge zincirine tasinmis durumda; Faz 8 bagimsiz audit revizyonuyla birlikte aktif-canvas anchor, canli MysteryMode referansi, tutorial resize-surface senkronu, popup backdrop recover zinciri, popup donusunde ana screen senkronu, screen-bound nesne rebind eksikleri, minimum workshop popup icerik butcesi, gameplay geometri helper'lari ve mod-ozel popup/particle stale-size zincirleri kapanmis oldu.
